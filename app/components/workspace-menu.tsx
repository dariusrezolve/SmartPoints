...
export function WorkspaceMenu({ childId, childName, childProfiles, currentWeekStart, initialManager, pointSummary, rewards, selectedTaskIds, taskCatalog, timeZone }: Props) {
  ...
  return <>
    <details className="group relative" ref={menuRef}>
      <summary className="flex cursor-pointer list-none items-center gap-2 rounded-2xl border border-white/80 bg-white/85 px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur-xl transition hover:-translate-y-0.5 hover:shadow-md [&::-webkit-details-marker]:hidden"><span className="grid size-8 place-items-center rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-sm">{childName.slice(0, 1).toUpperCase()}</span><MenuIcon aria-hidden="true" size={17}/>Menu <ChevronDown aria-hidden="true" className="text-slate-400 transition group-open:rotate-180" size={15}/></summary>
