  ...
  return (
    <header className="workspace-page mx-auto flex w-full max-w-5xl items-center justify-between gap-4 px-5 pt-6 sm:pt-8">
      ...
      <details className="group relative" ref={familyMenuRef}>
        <summary className="flex cursor-pointer list-none items-center gap-2 rounded-2xl border border-white/80 bg-white/85 px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur-xl hover:bg-emerald-50 [&::-webkit-details-marker]:hidden">
          <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white">{selectedChild?.display_name.slice(0, 1).toUpperCase() ?? "+"}</span>
        ...
        <Card className="absolute right-0 z-30 mt-3 w-[min(24rem,calc(100vw-2.5rem))] p-5">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-700">Your family</p>
          <h2 className="mt-2 text-lg font-semibold text-slate-950">Child profiles</h2>
          {childProfiles.length > 0 ? <nav aria-label="Child profiles" className="mt-4 flex flex-wrap gap-2">{childProfiles.map((child) => <Link aria-current={child.id === selectedChild?.id ? "page" : undefined} className={child.id === selectedChild?.id ? "rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 px-3 py-2 text-sm font-semibold text-white" : "rounded-full border border-emerald-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-emerald-50"} href={`/?child=${child.id}`} key={child.id}>{child.display_name}</Link>)}</nav> : <p className="mt-4 rounded-xl bg-emerald-50 px-3 py-2 text-sm text-emerald-900">Add the first child profile to start SmartPoints.</p>}
