...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, Gift, Sparkles, TrendingUp, WalletCards, WifiOff } from "lucide-react";
...
type Props = { parentId: string; childId: string; childName: string; childProfiles: Child[]; currentDate: string; currentWeekStart: string; initialNotice?: string; isCurrentWeek: boolean; pointSummary: PointSummary; taskCatalog: Task[]; tasks: Task[]; rewards: Reward[]; events: Event[]; timeZone: string };

export function PointsWorkspace({ parentId, childId, childName, childProfiles, currentDate, currentWeekStart, initialNotice, isCurrentWeek, pointSummary, taskCatalog, tasks, rewards, events, timeZone }: Props) {
  ...
  return <main className="workspace-page mx-auto w-full max-w-5xl px-5 pb-10 pt-6"><OfflineSnapshotWriter balance={optimisticPointSummary.balance} childId={childId} childName={childName} currentDate={currentDate} parentId={parentId} receivedThisWeek={optimisticPointSummary.receivedThisWeek} redeemedThisWeek={optimisticPointSummary.redeemedThisWeek} rewards={rewards} tasks={tasks}/>
    {toast ? <div aria-live="polite" className={`fixed inset-x-4 bottom-5 z-50 mx-auto flex max-w-md items-center gap-3 rounded-2xl border border-white/20 px-4 py-3 text-sm font-semibold text-white shadow-2xl backdrop-blur-xl ${toast.kind === "success" ? "bg-gradient-to-r from-emerald-600 to-teal-600" : toast.kind === "offline" ? "bg-gradient-to-r from-slate-700 to-slate-800" : "bg-gradient-to-r from-rose-600 to-red-600"}`} role="status">{toast.kind === "success" ? <CheckCircle2 aria-hidden="true" size={20}/> : toast.kind === "offline" ? <WifiOff aria-hidden="true" size={20}/> : <CircleAlert aria-hidden="true" size={20}/>}<span>{toast.message}</span></div> : null}
    <header className="mb-6">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.16em] text-emerald-700"><Sparkles aria-hidden="true" size={14}/>{isCurrentWeek ? currentDate : `Week of ${currentWeekStart}`}</p><h1 className="mt-2 bg-gradient-to-r from-slate-950 via-emerald-950 to-teal-800 bg-clip-text text-3xl font-extrabold tracking-tight text-transparent sm:text-4xl">{childName}&apos;s points</h1><div className="mt-3 flex flex-wrap items-center gap-2"><Link aria-label="Previous week" className="inline-flex items-center gap-1 rounded-xl border border-emerald-200/90 bg-white/80 px-3 py-2 text-sm font-semibold text-emerald-800 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:bg-emerald-50" href={`/?child=${childId}&week=${shiftWeek(currentWeekStart, -1)}`}><ChevronLeft aria-hidden="true" size={16}/>Previous</Link><Link aria-label="Next week" className="inline-flex items-center gap-1 rounded-xl border border-emerald-200/90 bg-white/80 px-3 py-2 text-sm font-semibold text-emerald-800 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:bg-emerald-50" href={`/?child=${childId}&week=${shiftWeek(currentWeekStart, 1)}`}>Next<ChevronRight aria-hidden="true" size={16}/></Link>{!isCurrentWeek ? <Link className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 px-3 py-2 text-sm font-semibold text-white shadow-sm" href={`/?child=${childId}`}><CalendarDays aria-hidden="true" size={15}/>Current week</Link> : null}</div></div>
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <Card className="p-5 sm:p-6"><h2 className="text-lg font-bold tracking-tight text-slate-950">Tasks for today</h2><p className="mt-1 text-sm text-slate-500">Tap a task to award points.</p>
      ...
      <Card className="p-5 sm:p-6"><h2 className="text-lg font-bold tracking-tight text-slate-950">Rewards</h2><p className="mt-1 text-sm text-slate-500">Redeem now, even when points go below zero.</p>
