...
function CachedWorkspace({ snapshot }: { snapshot: OfflineSnapshot }) {
  ...
  async function queueRewardUndo(eventId: string) { await submitPointAction({ childId: snapshot.childId, kind: "undo_reward", eventId, pointDelta: -(eventDeltas.get(eventId) ?? 0) }, "Reward redemption undone."); }
