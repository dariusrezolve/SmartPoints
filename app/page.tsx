...
import { getCurrentLocalDate, getResetPointSummary, getWeekStart, getWeeklyPointSummary } from "@/lib/points/validation";
...
export default async function HomePage({ searchParams }: HomePageProps) {
    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-xl items-center px-5 py-10">
        <Card className="p-6 sm:p-8">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-sky-600">SmartPoints foundation</p>
  ...
  const dailyTaskIds = new Set((weeklyTaskPlans ?? []).map((plan) => plan.task_id));
